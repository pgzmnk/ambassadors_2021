            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H5("Notes"),
                            html.H5(id='cps-concluding-remarks', children="Interactions, Incidents and Emergencies with CPS")
                        ])
                    ])
                ], width=12)
            ], className="mb-3")
        ...
        dbc.Col([
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.P("Map of CPS's Interactions, Incidents and Emergencies"),
                            dcc.Graph(id="cps-map1", config={'displayModeBar': True}
                                      )
                        ])
                    ])
                ], width=12),
...
# Update Map ***********************************************************
@app.callback(
    Output('cps-interactions','children'),
    Output('cps-incidents','children'),
    Output('cps-emergencies','children'),
    Output('cps-map1','figure'),
    Input('my-date-picker-start','date'),
    Input('my-date-picker-end','date'),
)
def update_graph(start_date, end_date):
    dff = df_preprocess[df_preprocess.creation_date.between(start_date, end_date)]
    dff = dff[dff['person_involved'].str.contains('CPS', regex=False)]
